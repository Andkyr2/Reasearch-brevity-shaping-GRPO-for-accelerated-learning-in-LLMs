...
4.  **Loss:** For each,
    $$L_{x}(\theta)=\frac{1}{k}\sum_{i=1}^{k}[-log~\pi_{\theta}(y_{i}|x)\cdot\hat{A}(x,y_{i})+\beta\cdot KL(\pi_{\theta}(\cdot|x)||\pi_{ref}(\cdot|x))]$$
    [cite_start]with sequence-level KL approximation[cite: 47]:
    $$KL_{sequence}\approx\frac{1}{|y_{i}|}(log~\pi_{\theta}(y_{i}|x)-log~\pi_{ref}(y_{i}|x))$$
...
$$R(x,y_{i})=max(0,1-\lambda\cdot max(0,|y_{i}|-35)) \text{ if correct, else 0,}$$
...
[cite_start]This schedule could be adaptive (e.g., based on length convergence) in future work[cite: 59].
...
[cite_start]The following table shows GSM8K test results after one epoch[cite: 73, 74].
...
[cite_start]The curriculum reduces lengths to < 80 tokens early, accelerating training, then recovers[cite: 116]. [cite_start]Without KL, the policy collapses; with it, the KL divergence stays low[cite: 116, 117]. [cite_start]High entropy in the brevity phase supports learning efficiency[cite: 117]. [cite_start]Overall, this results in 17.4% faster training with negligible accuracy loss[cite: 118].
...
*Caption: KL-divergence during curriculum training (low and bounded). [cite_start]The baseline without an explicit KL penalty reaches 0.2. [cite: 96]*
...
[1] Shenzhi Wang, Le Yu, Chang Gao, Chujie Zheng, Shixuan Liu, Rui Lu, Kai Dang, Xionghui Chen, Jianxin Yang, Zhenru Zhang, Yuqiong Liu, An Yang, Andrew Zhao, Yang Yue, Shiji Song, Bowen Yum, Gao Huang, Junyang Lin. Beyond the $80/20$ Rule: High-Entropy Minority Tokens Drive Effective Reinforcement Learning for LLM Reasoning. [cite_start]2025. [cite: 134, 135]
...
[3] J. Wei, X. Wang, D. Schuurmans, M. Bosma, F. Xia, E. Chi, Q. V. Le, D. Zhou. Chain-of-thought prompting elicits reasoning in large language models. [cite_start]In Advances in Neural Information Processing Systems, 2022. [cite: 137, 138]

[4] H. Lightman, V. Kosaraju, Y. Burda, H. Edwards, B. Baker, T. Lee, J. Leike, J. Schulman, I. Sutskever, K. Cobbe. [cite_start]Let's verify step by step. arXiv preprint arXiv:2305.20050, 2023. [cite: 139, 140]
...
[6] J. Schulman, F. Wolski, P. Dhariwal, A. Radford, O. Klimov. [cite_start]Proximal policy optimization algorithms. arXiv preprint arXiv:1707.06347, 2017. [cite: 143]
...
[8] X. Wang, J. Wei, D. Schuurmans, Q. Le, E. Chi, D. Zhou. [cite_start]Self-consistency improves chain of thought reasoning in language models. arXiv preprint arXiv:2203.11171, 2022. [cite: 146, 147]
